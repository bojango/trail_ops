...
def _moon_phase_name_from_fraction(phase: float | None) -> str | None:
    """Map phase fraction (0..1) to a human-friendly phase name.

    This is an approximation intended for UI labels, not astronomy.
    """
    if phase is None:
        return None
    try:
    ...
    # 8 buckets across the synodic month
...
def _upsert_legacy_weather_summary(conn, workout_id: int, wp_start: Any, wp_high: Any, *, start_time_utc: str | None, end_time_utc: str | None) -> None:
    ...
    data: dict[str, Any] = {
        "workout_id": workout_id,
        "point_type": "start",
        "obs_time_utc": getattr(wp_start, "obs_time_utc", None),
        "temp_c": getattr(wp_start, "temp_c", None),
        "wind_kph": getattr(wp_start, "wind_kph", None),
        "precip_mm": getattr(wp_start, "precip_mm", None),
        "weather_code": getattr(wp_start, "weather_code", None),
...
"moon_phase": getattr(wp_start, "moon_phase", None),
"moon_illumination": getattr(wp_start, "moon_illumination", None),
"moon_phase_name": _moon_phase_name_from_fraction(getattr(wp_start, "moon_phase", None)),
