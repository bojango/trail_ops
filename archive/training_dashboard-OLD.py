...
from analysis.route_context import get_route_context, get_surface_stats, get_peak_hits
from analysis.map_points import (
...
def _render_route_map(
    ...
    # Peaks visited (within configured distance)
    try:
        peaks = get_peak_hits(workout_id)
    except Exception:
        peaks = []
    ...
    if peaks:
        with st.expander(f"⛰ Peaks visited ({len(peaks)})", expanded=False):
            for p in peaks[:25]:
                name = p.name or f"OSM {p.peak_osm_id}"
                ele = f"{p.ele_m:.0f} m" if p.ele_m is not None else "ele N/A"
                dist = f"{p.distance_m:.0f} m"
                extra = []
                if p.wikidata:
                    extra.append(f"wikidata: {p.wikidata}")
                if p.wikipedia:
                    extra.append(f"wikipedia: {p.wikipedia}")
                extra_s = (" | " + " | ".join(extra)) if extra else ""
                st.write(f"- **{name}** ({ele}) · {dist}{extra_s}")
