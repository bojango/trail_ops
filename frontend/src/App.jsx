...
import { fetchHealth, fetchWorkouts, fetchSparklines, fetchWorkoutContext, fetchWorkoutMapPoints, fetchPeaksDashboard, fetchPeakItem } from "./api";
import PeaksPanel from "./peaks/PeaksPanel";
...
export default function App() {
  ...
  const [selectedPeakOsmId, setSelectedPeakOsmId] = useState(null);
  ...
  useEffect(() => {
    ...
    async function loadPeakItem() {
      if (selectedPeakOsmId == null) { setPeakItem(null); setPeakItemErr(null); return; }
      setPeakItemErr(null);
      setPeakItemLoading(true);
      try {
        const data = await fetchPeakItem(selectedPeakOsmId);
        if (!alive) return;
        setPeakItem(data);
      } catch (e) {
        if (!alive) return;
        ...
        <div className="brand">
          ...
          <div className="brandText">
            ...
            <div className="brandSub">Mission console * local-first * private</div>
                ...
                <div className="topFilters" style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ display: "flex", gap: 8, marginRight: 10 }}>
            <button className={"pillBtn " + (page === "dashboard" ? "accent" : "")} onClick={() => setPage("dashboard")}>
              Dashboard
            </button>
            <button className={"pillBtn " + (page === "planning" ? "accent" : "")} onClick={() => setPage("planning")}>
              Planning
            </button>
          </div>
          {/* RANGE chip */}
