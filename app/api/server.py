...
from app.api.planning_routes_phase2 import router as planning_routes_router
from app.api.planning_query_routes import router as planning_query_router
...
@app.get("/peaks/item")
def peaks_item_query(
    peak_osm_id: int = Query(..., description="OSM id for the peak/POI"),
    kind: Optional[str] = Query(None, description="Optional kind (peak/poi) for UI context"),
):
    """Diagnostics for a single peak/POI (query-param form)."""
    return get_peak_item(peak_osm_id)


@app.get("/peaks/item/{peak_osm_id}")
def peaks_item(peak_osm_id: int, kind: Optional[str] = Query(None)):
    """Diagnostics for a single peak/POI by peak_osm_id."""
    return get_peak_item(peak_osm_id)


@app.post("/peaks/item/{peak_osm_id}/rename")
def peaks_item_rename(peak_osm_id: int, body: RenameRequest, kind: Optional[str] = Query(None)):
    """Persist a user-defined name for a peak/POI."""
    return rename_peak_item(peak_osm_id, body.name)
...
def workout_map_points(
    ...
    # If the requested level is under-sampled (or missing), automatically pick the most-detailed
    # stored level available for this workout (largest point count). This avoids blocky routes.
    best_level = level
    try:
        best_df = df
        if best_df is None or best_df.empty or len(best_df) < 250:
            for lvl in (0, 1, 2, 3):
                if lvl == level:
                    continue
                cand = load_map_points(workout_id=workout_id, level=lvl)
                if cand is not None and not cand.empty and len(cand) > (0 if best_df is None else len(best_df)):
                    best_df = cand
                    best_level = lvl
            df = best_df
            level = best_level
...
# Planning (GPX Routes)
app.include_router(planning_routes_router)
app.include_router(planning_query_router)
