...
def get_raw_track_points(workout_id: int) -> pd.DataFrame:
    """Load raw GPS track directly from workout_samples (no simplification).

    Returned columns match get_map_points() output:
    seq, latitude_deg, longitude_deg, seconds_since_start, distance_m, elevation_m
    """
    raw = _load_raw_track(workout_id)
    if raw.empty:
        return pd.DataFrame(columns=["seq", "latitude_deg", "longitude_deg", "seconds_since_start", "distance_m", "elevation_m"])

    raw = raw.copy()
    raw.insert(0, "seq", range(len(raw)))

    # Coerce numeric defensively
    for c in ["seq", "latitude_deg", "longitude_deg", "seconds_since_start", "distance_m", "elevation_m"]:
        raw[c] = pd.to_numeric(raw[c], errors="coerce")

    raw = raw.dropna(subset=["latitude_deg", "longitude_deg"]).reset_index(drop=True)
    return raw
