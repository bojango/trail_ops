...
def get_peaks_dashboard(
    ...
    try:
        ...
        for r in dim_rows:
            pid = int(r["peak_osm_id"])
            cls_raw = r.get("dobih_classifications") if hasattr(r, "get") else r["dobih_classifications"]
            cls_list = []
            if cls_raw:
                try:
                ...
                top_peaks.append(
                    {
                        ...
                        "last": (str(r.get("last_visit"))[:10] if r.get("last_visit") else None),
                    }
                )
