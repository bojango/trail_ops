...
@app.get("/peaks/item", dependencies=[Depends(api_key_dependency)])
def peaks_item_query(
    peak_osm_id: int = Query(...),
    kind: str = Query("peak"),
):
    """Diagnostics for a single peak/POI by peak_osm_id (query form)."""
    return get_peak_item(peak_osm_id, kind=kind)


@app.get("/peaks/item/{peak_osm_id}", dependencies=[Depends(api_key_dependency)])
def peaks_item(peak_osm_id: int, kind: str = Query("peak")):
