...
def get_peaks_dashboard(
    ...
    try:
        ...
        # 5) Class trackers + top visited list for selected class
        peak_cols = _cols(conn, "peaks")
        class_col = "dobih_classifications" if "dobih_classifications" in peak_cols else None

        # Load peaks dimension (only fields we need)
        dim_rows = conn.execute(
            """
            SELECT peak_osm_id, name, ele_m, dobih_classifications
            FROM peaks
            """
        ).fetchall()
        ...
        for r in dim_rows:
            ...
            for c in cls_list:
                ...
                if not key:
                    continue
                totals_by_class[key] = totals_by_class.get(key, 0) + 1
                ids_by_class.setdefault(key, set()).add(pid)

        # Visited unique peaks per class (all-time)
        visited_rows = conn.execute(
            """
            SELECT DISTINCT peak_osm_id
            FROM workout_peak_hits
            WHERE hit_type = 'bagged:peak'
            """
        ).fetchall()
        visited_set = {int(r["peak_osm_id"]) for r in visited_rows}

        # Build trackers for a shortlist of classes we actually have in the dim table
        # Prefer a stable ordering: selected class first, then others by total desc.
        ...
        # Top visited peaks within selected class (all-time)
        # If selected class has no mapping, just show top visited overall peaks.
        ...
        top_rows = conn.execute(
            ...
            SELECT
              h.peak_osm_id,
              COUNT(*) AS hits,
              MAX(w.start_time) AS last_visit
            FROM workout_peak_hits h
            JOIN workouts w ON w.id = h.workout_id
            WHERE h.hit_type = 'bagged:peak'
            {where_in}
...
def get_peak_item(peak_osm_id: int, kind: str = "peak") -> dict:
    ...
    # hits summary (schema-drift safe)
    cur.execute(
    """
    SELECT
        SUM(CASE WHEN h.hit_type LIKE 'bagged:%' THEN 1 ELSE 0 END) AS bagged_count,
        SUM(CASE WHEN h.hit_type LIKE 'near:%' THEN 1 ELSE 0 END) AS near_count,
        MAX(CASE WHEN h.hit_type LIKE 'bagged:%' THEN w.start_time ELSE NULL END) AS last_bagged,
        MAX(CASE WHEN h.hit_type LIKE 'near:%' THEN w.start_time ELSE NULL END) AS last_near
    FROM workout_peak_hits h
    JOIN workouts w ON w.id = h.workout_id
    WHERE h.peak_osm_id = ?
    """,
