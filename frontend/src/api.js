...
// Persist a human-friendly name to the peaks table.
// Backend route: POST /peaks/item/{peak_osm_id}/rename
export async function renamePeakItem(peakOsmId, name, kind = "peak") {
  const id = encodeURIComponent(String(peakOsmId));
  return httpJson(`/peaks/item/${id}/rename${qs({ kind })}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
